...
.DESCRIPTION
  Report-only Intune Remediation script build: v4.3.0
  - Always exits 0
...
.NOTES
  ...
  Adds retry logic for downloads/uploads; prefers DCU feed (CatalogIndexPC -> per-SKU manifest) then fallback catalogs; richer HTTP error reporting.
#>
...
if (-not [string]::IsNullOrWhiteSpace($TimeStampField)) {
    $headers["time-generated-field"] = $TimeStampField
        ...
        try {
            $resp = Invoke-WebRequestWithRetry -Uri $uri -Method $method -ContentType $contentType -Headers $headers -Body $BodyBytes -UseBasicParsing -ErrorAction Stop
...
function Get-XmlFromCabUrl {
    ...
    if (-not (Test-Path $cabPath)) {
        Invoke-WebRequestWithRetry -Uri $Url -UseBasicParsing -OutFile $cabPath -ErrorAction Stop | Out-Null
    }
